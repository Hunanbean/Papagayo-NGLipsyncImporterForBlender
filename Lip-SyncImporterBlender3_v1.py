...
def createShapekey(phoneme, frame):
    ...
    if lastPhoneme!=phoneme or eval(scn.regMENU_PG_types.enumModeTypes) == 1:
        ...
        objSK.key_blocks[phoneme].keyframe_insert(data_path="value",
            index=-1, frame=offst+frame-frmIn, group="Lipsync")

    objSK.key_blocks[phoneme].value=skVlu
    objSK.key_blocks[phoneme].keyframe_insert(data_path="value",
        index=-1, frame=offst+frame, group="Lipsync")

    objSK.key_blocks[phoneme].value=skVlu
    objSK.key_blocks[phoneme].keyframe_insert(data_path="value",
        index=-1, frame=offst+frame+hldIn, group="Lipsync")

    objSK.key_blocks[phoneme].value=0.0
    objSK.key_blocks[phoneme].keyframe_insert(data_path="value",
    index=-1, frame=offst+frame+hldIn+frmOut, group="Lipsync")
...
class MENU_PG_types(bpy.types.PropertyGroup):
    enumModeTypes : EnumProperty(name = 'Choose Mode',
                                 items =(('0', 'Lipsyncer',''),
                                         ('1', 'Blinker','')),
                                 default = '0')
